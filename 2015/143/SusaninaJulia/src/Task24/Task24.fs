...
let time f =
    ...
    for i = 1 to 10 do f()
    (System.DateTime.Now - start).TotalMilliseconds / 10.00
...
let main =
    Chart.Combine
        [
            ...
            Chart.Line( [ for i in 1..2..1500 -> (i, time(fun () -> List.sort([for k in 1..i ->rand.Next()]) |> ignore) ) ], "List.sort", Color = System.Drawing.Color.DarkKhaki)
        ]
